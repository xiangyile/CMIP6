  ...
  if (models[i] != "FGOALS-g3") { # FGOALS-g3等模式与一般模式格式不一致，需要运行单独代码，详见else
    ...
    for (j in 1:length(SSPs)) {
      ...
      for (k in 1:length(files)) {
        ...
        m <- 0
        for (i_lon in 1:length(lon)) { # 115:136为松辽流域经度范围
          if (lon[i_lon] >= 115 &lon[i_lon] <= 136) { # 与上一致
            for (i_lat in 1:length(lat)) { # 38:54为松辽流域纬度范围
              if (lat[i_lat] >= 38 &lat[i_lat] <= 54) { # 与上一致
                m <- m+1 
                prseries <- as.data.frame(ncvar_get(nc_data,varid = 'pr')[i_lon,i_lat,]*3600*24) # 提取的序列
                startyear <- 1964+k-1 # 以FGOALS-g3为例，每个nc文件包含一年的数据，如果其他模式格式不同，此行需要修改
                ...
                prseries2 <- prseries
                info <- data.frame(V1 = c(m, round(lon[i_lon],2), round(lat[i_lat],2)))
                rownames(info) <- c("ID", "lon", "lat")
                colnames(info) <- "pr"
                prseries2 <- rbind(info, prseries2)
                rm(info)
...
print("全部提取完毕")
write.table(data.frame(error_files), "错误文件.txt", col.names = F, row.names = F)