...
# CMIP6数据分为多模式和多情景，代码中以EC-Earth3和FGOALS-g3两个模式的historicl和SSP126两种情景为例
    ...
    if (models[i] != "FGOALS-g3") { # FGOALS-g3等模式与一般模式格式不一致，需要运行单独代码，详见else
        ...
        for (j in 1:length(SSPs)) {
            ...
            for (k in 1:length(files)) {
                ...
                # 裁剪出松辽流域内的nc文件
                raster_nc <- raster::brick(raster(file1)) # 读取原始的全球范围nc文件
                ...
                for (m in 1:length(grids_nc$lon)) {
                    ...
                    if (m == 1) {
                        ...
                        prseries1 <- rbind(info, prseries1)
                        rm(info)
                    }else{
                        prseries1 <- rbind(prseries1, prseries)
                    }
                }
                ...
                if (k == 1) {
                    ...
                    prresult <- cbind(prresult, prseries1)
                }
            }
            setwd(res_dir)
